...
Every ticket planned its seams and its expected values, and nothing planned its edge cases: the empty list, the value on the boundary, the second run, the error a dependency signals. Tests written from the happy path did not reach them, and mutation testing, which shows which code no test constrains, ran only on request with a command that mutmut 3 no longer accepts. Asked for: make finding edge cases part of the agent's testing and writing philosophy. Decision 54.

- **Edge cases are listed, decided and tested in every ticket.** New `py-design/references/edge-cases.md`: a type rules out what it can (rule 14: a constrained field, an enum, a `NewType`, a constructor that rejects the bad state, so one test at the parse replaces a check in every caller); error paths and short sequences are behaviours; an expected value nobody decided is a grill question, never a guess; edge cases are tested through the ticket's seams, in proportion, the way Matt Pocock's `tdd` asks. Nine categories per behaviour (values, types and formats, domain values, calls out, state and sequence, concurrency, scale, trust, configuration), the Python traps, and the ticket's `## Edge cases` table: one line naming the categories gone through and why any were skipped, then a row per edge case decided as type, test, question or out of scope, with the source of its expected value and its test. A ticket with an open question row is not published.
- **The flow runs it.** `py-shape` lists each behaviour's edge cases as the grill settles it; `py-build` re-checks the list against the code before the go, tests each edge case in the slice that owns it, and runs `mutmut` on the changed modules before review, killing each survivor with a test or recording it in one line as equivalent or noise; `py-review` reads the ticket's table against the tests diff (a decided edge case with no test is 🔴, a missing property test or an untriaged survivor 🟠); `py-reviewer` looks for the faults only judgement catches. The persona says a plan names the edge cases its tests cover, and that an undecided one is a decision for the user; two clauses, about 13,560 characters (ceiling 14,000). `py-design`'s `## The edges` is now `## Boundaries`, because "edge" here means a system boundary; `CONTEXT.md` gains **Edge case**.
- **Property tests, with Hypothesis in the baseline.** A transform or serialiser with a rule that holds for every valid input gets a Hypothesis property test beside its example tests (round trip, invariant, idempotence, reference model, metamorphic), never a validator, no quota; the rules against the ways agents get them wrong (`assume()` that filters the bug away, a strategy narrower or wider than the spec, an oracle that calls the code under test) are in the reference and the fault catalogue. `hypothesis>=6.154` joins the dev group: 6.154.0 is the first release whose built-in `ci` profile makes a CI failure reproduce without a conftest (bisected on PyPI). The intake report names it and mutmut.
- **The mutmut commands and keys match mutmut 3.8.** `mutmut run --paths-to-mutate` was a usage error; a module is selected by a mutant-name glob (`mutmut run "<package>.<module>*"`), and the template's keys are `source_paths` and `pytest_add_cli_args_test_selection` (the old names only warned). Intake ignores `mutants/` and `.hypothesis/` instead of the 2.x `.mutmut-cache/`. A repo set up earlier keeps its old keys and gains the new ones at the next intake, since intake merges missing keys only; mutmut warns about the old ones until they are removed. The fault catalogue and ADR 0002 stop citing a mutation-score gate that never existed.
- **The pack example tests its edge cases.** Mutation on it (391 mutants, seven seconds) found eight untested edge cases in a suite that had passed a hand-made bug check: a tie on the update time, a zero amount, an empty order id, a run with nothing new, a short CSV row, each required command-line argument. Each has a test now, and the transform has two property tests; 54 tests. Of the 75 remaining survivors none is a missing test, and about three in four are message text or keyword arguments, which is why triage has a skip-on-sight outcome and nothing is gated on a score (`docs/research/edge-cases.md`).
- **CI:** `check_pack_examples.py` runs with hypothesis installed. `docs/smoke-test.md` gains five lines in sections 2 and 5.
- Smoke test for this release: not yet run on a live model.
