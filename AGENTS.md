...
This repo *is* the plugin. `.claude/settings.json` selects the installed copy from the plugin cache; to run the working tree's persona and skills against this repo, start with `claude --agent python-dev --plugin-dir .`. The product code is `scripts/` (the hooks and the plugin checks); `skills/`, `packs/` and `agents/` are shipped Markdown and templates, checked by `scripts/check_plugin.py`, `scripts/check_pack_examples.py` and `scripts/check_template_deps.py`, not by this repo's ruff and mypy.
