...
| `skills/py-design` | The craft rules, the order to build things in (`references/planning.md`), the boundaries: timeouts, retries, parsing, errors, config, logs, contract tests (`references/boundaries.md`), the edge-case method: nine categories per behaviour, the ticket's table, property-test rules, mutation triage (`references/edge-cases.md`), the fault catalogue, three canonical repos to cite, the worked adapter-model-writer shape, the test-audit classes. |
