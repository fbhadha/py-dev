...
| `skills/py-design` | The craft rules, the order to build things in (`references/planning.md`), the edges: timeouts, retries, parsing, errors, config, logs, contract tests (`references/boundaries.md`), the fault catalogue, three canonical repos to cite, the worked adapter-model-writer shape, the test-audit classes. |
...
What a harness loads every turn from this plugin is the persona and nine skill descriptions, about 4,100 tokens at 3.6 characters per token (persona ~3,300, descriptions ~800), plus the guard line from the session-start hook. 0.11.0 spends about 700 more tokens a turn than 0.10.0 on the loop, the explaining and the pushback, which the 10,000-character ceiling had squeezed out; CI now fails at 14,000, so it cannot creep further without a choice. Skill bodies load only when a skill runs (`py-intake`, the largest, about 7,000 tokens, once per repo; `py-shape` about 2,400, once per idea; `py-build` about 1,700, once per ticket); `references/` files only when a skill opens them. Matt's skills add their descriptions when installed. Google's four are installed only in ADK repos. The rest of the session's overhead is what the persona reads and runs before real work, and each of those is bounded: session start reads `AGENTS.md` only (`CONTEXT.md`, the tracker file and a how-to are opened when a step needs them); the index is refreshed only when `repowise status` says it is behind HEAD; the upstream door check caches a clean result for a day; one `risk` call covers every file a ticket touches; test, pre-commit and log output goes through `repowise distill`, which drops the passing noise and keeps the failures; intake's orientation reads at most three entry points. Every step is a numbered instruction or a command, so a smaller model can follow it; what a smaller model does worse is the judgement in the review's Craft axis and in grilling, and the mechanical checks do not get weaker.
