...
**Every session after that.** The persona reads the repo's state, checks the upstream skills are installed, says in one line what comes next, and starts it. An idea is shaped before anything is built (`py-shape`), on a `shaping/` branch that holds decisions only: Matt's `grill-with-docs`, with his `research` (a background reader that leaves a dated, cited note in `docs/research/`) for a fact from outside the repo and his `prototype` (throwaway code on a `prototype/` branch, never merged) for a question talking cannot settle; an idea bigger than one session becomes a `wayfinder` map of decision tickets on the tracker, one per session, that hands off to `to-spec` when it clears. Then `to-spec` and `to-tickets` with a handoff and a fresh session between each. A ticket gets built (`py-build`): name the shape, ask Repowise what the files depend on, search before naming anything new, agree the seams, then Matt's `implement` with `tdd`, checks after every green, the change gate, a four-axis review (`py-review`), a commit that names the decision, a handoff. One ticket per session.
...
| Path | What |
|---|---|
| `agents/python-dev.md` | The persona, the only hand-written copy, and the router: session start, the routing table (situation, what to run), session boundaries, the Repowise policy, voice, pushback, the branch rules, what to do when the plugin is wrong. Always loaded; CI keeps it under 10,000 characters and checks every skill is reachable from it. |
...
| `skills/py-shape` | An idea to its ticket: the grill, a research note, a prototype, a wayfinder map when it is more than one session, on a `shaping/` branch that holds decisions only. |
| `skills/py-build` | A ticket to its merge: the shape, Repowise, the seams, Matt's `implement` with `tdd`, the gates, the review, the merge question, the handoff. |
...
| `upstream.json`, `scripts/find_skill.py` | Every upstream skill called by name and the dictionary terms the persona links to, pinned; and the locator that finds an installed skill's `SKILL.md` wherever a harness put it. |
...
| Shaping never builds | On a `shaping/` branch the command hook asks before a commit that carries `src/` or `tests/` files. | Product code written inside a grill or a wayfinder map, the failure Matt's own docs report most |
...
The guards are on as soon as the plugin is installed; a session-start hook prints `python-dev guards active` with the current branch, and the persona checks for it. `PYTHON_DEV_GUARD=off` silences the ask for one session; the denies stay. The hooks fail open on any error of their own, and `scripts/test_hooks.py` drives every decision against a scratch repo in CI.
...
Rules: `agents/python-dev.md` is the only hand-written persona, and it is a router: the routing table and the rules that hold on every turn, under 10,000 characters, which CI enforces along with every skill being reachable from it; every step list is a skill the table names; every other copy is generated and CI fails when it is stale. Call upstream skills by name, never copy them; add the name to `upstream.json` and bump a pin in its own commit after reading the upstream changelog. One read path and one write path per kind of knowledge (ADR 0005): anything derived from the code comes from Repowise, by CLI. No custom code-quality gates; a check is an established tool's rule in `skills/py-baseline/templates/pyproject-tools.toml`. The four scripts the baseline copies into a repo (change gate, ADR binder, README runner, test-diff check) are process, not lint. Verify before you write; `docs/research/` records what was checked and when. A knowledge pack is `skills/pack-<domain>/` in the shape of `packs/TEMPLATE.md`, reference only. Before a release: the five commands above, a smoke test in a real repo on each harness, then the version in every manifest and a `CHANGELOG.md` entry.
