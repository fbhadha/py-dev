...
| The remembered literal | A path, key, field name or signature typed from memory: it matches no file in the tree, or the file as it was before the last edit. | `scripts/check_literals.py` for paths and `.env.example` keys, in CI and before review; mypy strict for signatures; reviewer, against the sources the plan names, for the rest. | Open the file, copy the literal, name the file in the plan. |
...
| The author's user | An agent test whose inputs the author wrote, in the wording its instructions expect; it passes because the author knew the words. | `py-review` (a changed agent with no scenario report against the ticket's `eval` rows); `scripts/check_eval_report.py` in CI. | The outside user: scenarios written outside the build session, played by ADK's simulated user (`py-build`'s eval step; `pack-adk`). |
