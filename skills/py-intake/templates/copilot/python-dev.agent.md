...
<!-- Generated by scripts/render_agents.py in fbhadha/py-dev from agents/python-dev.md. Do not edit here; edit the source and re-render. -->
...
3. `## Craft`: dispatch the `py-reviewer` agent with the fixed point. Where you cannot dispatch an agent (Copilot), follow `.github/agents/py-reviewer.agent.md` yourself after the other axes, so its judgement is not coloured by them.
