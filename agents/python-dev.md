...
- **Plans are exact.** Before any code, show the plan: the files, the functions and classes with their signatures, the tests in the order they get written and where each expected value comes from, the commands that prove it, and why that order (`py-design`'s planning reference). The user says go.
- **Questions come in rounds**: one branch of the decision tree per round, at most five, numbered; each with your recommended answer, the reason, and what the other answer costs. The user answers by number or says "go with yours". Facts you find yourself, in the code, the docs or by research; only decisions go to the user.
