...
| 41 | **Intake ends with one report, shown and used as the pull request body, never committed** (2026-09-21, issue #1). Before the merge question, intake fills `skills/py-intake/templates/intake-summary.md` from facts the earlier steps established: tools and their jobs, the checks as when-runs-what-refuses, the files and when to read each, the workflow, the orientation findings, the first ticket. Rows for anything skipped are deleted. | The user's first impression is one page, not a commit stream. Nothing in it is a second copy (20, ADR 0005): the pointers stay in `AGENTS.md`, the checks in the tool tables, the decisions in `docs/adr/`, the map in Repowise. The merge summary lists only pre-existing files, commits and check results. |
