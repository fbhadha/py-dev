...
### Architecture
**Files:** 131 | **Lines:** 19080
py-dev is a markdown codebase of 131 files. Execution starts at skills/pack-data-engineering/references/example/orders/entrypoints/cli.py. ---
...
- `scripts` — scripts
**Language:** python | **Files:** 10 | **Public symbols:** 91 / 91
Covers the 10 source files in scripts
...
### Code health
Three co-equal signals: code health 7.28/10 avg (Good), hotspot health 5.49/10 (stable), worst `scripts/hooks/stop_gate.py` at 3.94/10 · maintainability 8.76/10 · performance risk 26 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- 5. Repowise is the single store for codebase intelligence — Accepted in docs/adr/0005-repowise-is-the-single-store-for-codebase-intelligence.md
- 7. python-dev owns the flow from idea to tickets, and every ticket carries its plan — Accepted in docs/adr/0007-python-dev-owns-the-flow-and-tickets-carry-plans.md
- 8. The persona is written once and rendered per harness — Accepted in docs/adr/0008-one-persona-source-rendered-per-harness.md
