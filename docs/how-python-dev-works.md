...
The plugin's hooks run in the session on every harness it declares them for. A pre-tool hook makes the harness ask you before the agent changes an existing protected file (in the default `ask-once` mode the first yes covers the session; in `on` mode it is once per file), and denies destructive git outright. A stop hook refuses to end the turn while a protected file is modified without that approval, or while ruff is red on files the session changed. In your repo, a commit-msg hook refuses a commit that touches a protected file unless the message names it as approved. The guards are on by default; `protect-existing-files` in `docs/agents/mode.md` picks `ask-once`, `on` or `off`, and `PYTHON_DEV_GUARD=off` turns the file guard off for a session.
