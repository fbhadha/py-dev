...
The plugin's hooks run in the session on every harness it declares them for. The rule they hold is one line: `main` changes only by a merge you said yes to. The agent works on a branch per ticket; a pre-tool hook makes the harness ask you before a commit, merge or push that lands on `main`, and denies destructive git outright; a stop hook refuses to end the turn while ruff is red on files the session changed. In your repo, CI runs the test-diff check and coverage on changed lines on every pull request, and branch protection on the server (set at intake with your yes) refuses a merge until they are green. `PYTHON_DEV_GUARD=off` silences the ask for one session.
