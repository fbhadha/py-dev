...
5. `python3 "${PLUGIN_ROOT}/scripts/find_skill.py" --door-check`. Report anything missing with its install line. Never improvise a missing skill.
...
3. `## Craft`: dispatch the `py-reviewer` agent with the fixed point. Where you cannot dispatch an agent (Copilot), follow `com.github.copilot/agents/py-reviewer.agent.md` yourself after the other axes, so its judgement is not coloured by them.
